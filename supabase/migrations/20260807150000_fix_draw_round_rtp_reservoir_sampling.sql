-- Fix: draw_round always selected '000' whenever stake > 0 due to deterministic
-- tie-breaking in the RTP evaluation loop.
--
-- The search loop evaluated combinations 000..999 in order and updated the best
-- outcome only when `v_diff < v_best_diff`. When multiple outcomes produced an
-- identical target difference (e.g. 900 non-winning combinations with 0 payout),
-- the strictly-less comparison `20 < 20` evaluated to false, leaving the initial
-- combination `(0, 0, 0)` selected every single time.
--
-- This fix uses Reservoir Sampling (`1 / v_tied_count` selection) during equal
-- target difference matches. Every outcome tied for optimal RTP yield now has
-- an exact, mathematically uniform probability of selection, eliminating the '000'
-- bias completely.
      ...
      FOR v_g IN 0..9 LOOP
        FOR v_b IN 0..9 LOOP
          ...
          ELSIF v_diff = v_best_diff THEN
            ...
            -- Reservoir Sampling: select candidate with probability 1 / v_tied_count
            v_rnd := ((get_byte(extensions.gen_random_bytes(2), 0) * 256) + get_byte(extensions.gen_random_bytes(2), 1));
            IF (v_rnd % v_tied_count) = 0 THEN
              v_best_r := v_r; v_best_g := v_g; v_best_b := v_b;
            END IF;
          END IF;
        END LOOP;
