-- ============================================================================
-- BEST SMART GAME (BSG) — FULL DATABASE RESET & CLEAN REBUILD SCRIPT
-- WARNING: THIS SCRIPT DROPS ALL EXISTING TABLES AND DATA IN PUBLIC SCHEMA
-- AND RE-CREATES A 100% CLEAN, PERFECT SYSTEM SCHEMA FROM SCRATCH.
-- Copy & Paste this entire script into your Supabase Dashboard SQL Editor.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- STEP 1: DROP ALL EXISTING PUBLIC TABLES, TRIGGERS & FUNCTIONS (CASCADE)
-- ----------------------------------------------------------------------------
DROP TRIGGER IF EXISTS trg_sync_user_metadata_balance ON public.profiles CASCADE;
DROP FUNCTION IF EXISTS public.sync_user_metadata_balance() CASCADE;
...
DROP TABLE IF EXISTS public.agent_coin_transactions CASCADE;
...
-- Drop RPC functions
DROP FUNCTION IF EXISTS public.get_effective_rtp(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.get_system_target_rtp() CASCADE;
DROP FUNCTION IF EXISTS public.issue_agent_coins(uuid, uuid, numeric, text) CASCADE;
DROP FUNCTION IF EXISTS public.issue_agent_coins(uuid, numeric) CASCADE;
DROP FUNCTION IF EXISTS public.transfer_coins_agent_to_player(uuid, uuid, numeric) CASCADE;
DROP FUNCTION IF EXISTS public.transfer_coins_agent_to_player(uuid, numeric, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.withdraw_coins_player_to_agent(uuid, uuid, numeric) CASCADE;
DROP FUNCTION IF EXISTS public.withdraw_coins_player_to_agent(uuid, numeric, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.check_and_update_login_session(uuid, text) CASCADE;
DROP FUNCTION IF EXISTS public.get_current_round() CASCADE;
DROP FUNCTION IF EXISTS public.submit_round_bet(uuid, jsonb, jsonb, jsonb, numeric) CASCADE;
DROP FUNCTION IF EXISTS public.get_my_round_result(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.get_play_limits() CASCADE;
DROP FUNCTION IF EXISTS public.get_recent_rounds(int) CASCADE;
...
-- ----------------------------------------------------------------------------
-- STEP 2: CREATE ALL 10 CLEAN SYSTEM TABLES
-- ----------------------------------------------------------------------------
...
-- Seed global system RTP default row
INSERT INTO public.agent_configs (id, agent_id, rtp, rtp_percentage, target_win_percentage, updated_at)
VALUES ('global_system_config', NULL, 96.50, 96.50, 96.50, NOW())
ON CONFLICT (id) DO UPDATE SET rtp = 96.50, rtp_percentage = 96.50, target_win_percentage = 96.50;
...
CREATE TABLE public.agent_coin_transactions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ----------------------------------------------------------------------------
-- STEP 3: CREATE ALL SECURITY DEFINER RPC FUNCTIONS WITH ALL OVERLOADS
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- STEP 5: SYNC PROFILES FOR ANY EXISTING AUTH USERS (IF ANY)
-- ----------------------------------------------------------------------------
...
SELECT 
  ...
  COALESCE((raw_user_meta_data->>'balance')::numeric, 0.00),
  TRUE
FROM auth.users
...
-- ----------------------------------------------------------------------------
-- STEP 6: RELOAD SCHEMA CACHE IMMEDIATELY
-- ----------------------------------------------------------------------------
