...
export async function getAgentDashboardDataAction() {
  ...
  try {
    ...
    const [agentProfRes, playersProfRes, spinsRes] = await Promise.all([
      ...
      supabaseAdmin.from('profiles').select('id, username, balance, is_active').or(`agent_id.eq.${authUser.id},parent_agent_id.eq.${authUser.id}`),
    ...
    const balance = agentProfRes.data ? Number(agentProfRes.data.balance || 0) : 0
    const username = agentProfRes.data?.username || authUser.username || 'agent'
    const agentName = username
    const joinedDate = 'Jul 28, 2026'
    ...
    if (playersProfRes.data && playersProfRes.data.length > 0) {
      ...
      players = allUsers
        .filter(u => u.user_metadata?.role === 'player' && (u.user_metadata?.agent_id === authUser.id || u.user_metadata?.parent_agent_id === authUser.id))
        .map(u => ({
          id: u.id,
